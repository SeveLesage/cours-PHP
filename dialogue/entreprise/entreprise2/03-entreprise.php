...
/* -> se base sur ou s'appuie sur */
/* => élément à gauche correspond à l'élément à droite */

// 3- Réception des infos d'un employé avec $_GET
if (isset($_GET['id_employes'])) { // isset est-ce que id-employes est prédéfini?
    ...
    if ($resultat->rowCount() == 0) { /*ceci veut dire que si la personne est venue sur la page en récupérant un id _employe qui n'existe pas , alors renvoyer sur la pge 02 */
        header('location:02-entreprise.php');
        exit();
    }
    $fiche = $resultat->fetch(PDO::FETCH_ASSOC);/* Sur notre variable $résultat qui contient notre requête (ici on sélectionne toutes les infos d'un employé donné) on demande fetch (va chercher) et on lui indique qu'il faut récupérer les infos dans la BDD
    FETCH_ASSOC permet de renvoyer les résultats d'une rangée comme venant d'un tableau*/
} else {/* si la personne vient sur la page juste 03-entreprise.php on la renvoievers la page 02-entreprise.php // doit etre du if principal car on demande de sortir si on ne récupère pas d'id_employes dans */
    header('location:02-entreprise.php');
    exit();
}
                ...
                <form action="#" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="service" id="service" class="form-select">
                            ...
                            ?><!-- Ici on sélectionne tous les services qui existent dans la BDD et on les affiche en tant qu'option avec la valeur à laquelle l'option correspond -->
                        </select>
                    </div>
